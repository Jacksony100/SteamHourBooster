    ...
    <main className="min-h-screen">
      <div className="mx-auto grid w-full max-w-6xl items-center gap-8 px-6 py-12 md:min-h-[calc(100vh-96px)] md:grid-cols-[1.2fr_0.8fr]">
        <section className="space-y-6">
          ...
          <div className="w-fit rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-sm font-semibold text-emerald-200">{t.auth.registerEyebrow}</div>
          <h1 className="text-gradient text-5xl font-black md:text-7xl">{t.auth.registerHeadline}</h1>
          <p className="max-w-xl text-lg leading-8 text-slate-300">{t.auth.registerBody}</p>
          <Link className="text-sm font-semibold text-sky-200" href="/login">
            {t.auth.hasAccount}
          </Link>
        </section>
